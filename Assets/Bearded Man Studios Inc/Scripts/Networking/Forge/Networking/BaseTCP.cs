...
namespace BeardedManStudios.Forge.Networking {
    ...
    public abstract class BaseTCP : NetWorker {
        ...
        protected byte[] HandleHttpHeader(SocketAsyncEventArgs e, ref int bytesAlreadyProcessed) {
            ...
            if (totalBytes >= 4) {
                    ...
                    if (HEADER_DELIM[0] == data[token.internalBuffer.Offset + i]) {
                        if (HEADER_DELIM[1] == data[token.internalBuffer.Offset + i + 1] &&
                            ...
                            if (header.Length < totalBytes) {
                                Buffer.BlockCopy(data, token.internalBuffer.Offset + header.Length, data,
                                    token.internalBuffer.Offset, totalBytes - header.Length);
                            }
                ...
                if (totalBytes == token.internalBuffer.Count)
                    throw new Exception(String.Format("Could not find end of header within {0} bytes.",
                        token.internalBuffer.Count));
            }
        ...
        protected byte[] HandleData(SocketAsyncEventArgs e, bool isStream, ref int bytesAlreadyProcessed) {
            ...
            if (token.dataHolder == null) // Null dataHolder means header not parsed yet
            {
                    ...
                    if (payloadOffset == 4) {
                        if (BitConverter.IsLittleEndian)
                            length = BitConverter.ToUInt16(
                                new byte[] {bytes[socketOffset + 3], bytes[socketOffset + 2]}, 0);
                        else
                        ...
                        if (BitConverter.IsLittleEndian)
                            length = (int) BitConverter.ToUInt32(
                                new byte[] {
                                    bytes[socketOffset + 9], bytes[socketOffset + 8], bytes[socketOffset + 7],
                                    bytes[socketOffset + 6]
                                }, 0);
                        else
                ...
                if (length < 0 || length > token.maxAllowedBytes)
                    throw new UnauthorizedAccessException(String.Format(
                        "Tried to receive a frame larger than expected. Got {0}, and expected less than {1}", length,
                        token.maxAllowedBytes));
                e.SetBuffer(token.internalBuffer.Offset, token.internalBuffer.Count);
                token.bytesReceived = 0;
            ...
            {
                Buffer.BlockCopy(bytes, socketOffset, token.dataHolder, token.bytesReceived,
                    e.BytesTransferred - bytesAlreadyProcessed);
                ...
                ) // More frames left to handle, so shuffle the remaining data to beginning of buffer.
                {
                    Buffer.BlockCopy(bytes, socketOffset + dataProcessed, bytes, socketOffset,
                        e.BytesTransferred - bytesAlreadyProcessed);
                }
