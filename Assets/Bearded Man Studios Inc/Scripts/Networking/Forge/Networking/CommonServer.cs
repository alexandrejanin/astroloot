...
namespace BeardedManStudios.Forge.Networking {
    public class CommonServerLogic {
        ...
        public bool PlayerIsReceiver(NetworkingPlayer player, FrameStream frame, float proximityDistance,
            NetworkingPlayer skipPlayer = null, int proximityModeUpdateFrequency = 0) {
            ...
            if (player == frame.Sender) {
                ...
                if (frame.Receivers == Receivers.Others || frame.Receivers == Receivers.OthersBuffered ||
                    frame.Receivers == Receivers.OthersProximity || frame.Receivers == Receivers.OthersProximityGrid)
                    return false;
            }

            // check if sender is null as it doesn't get sent in certain cases
            if (frame.Sender != null) {
                return PlayerIsDistanceReceiver(frame.Sender, player, frame, proximityDistance,
        ...
        public bool PlayerIsDistanceReceiver(NetworkingPlayer sender, NetworkingPlayer player, FrameStream frame,
            float proximityDistance, float proximityModeUpdateFrequency) {
            ...
            if (sender != null) {
                ...
                } else if ((frame.Receivers == Receivers.AllProximityGrid ||
        ...
        private bool proximityDistanceCheck(NetworkingPlayer sender, NetworkingPlayer player, float proximityDistance,
            float proximityModeUpdateFrequency) {
            // If the target player is not in the same proximity zone as the sender
            // then it should not be sent to that player
            if (player.ProximityLocation.DistanceSquared(sender.ProximityLocation) >
        ...
        private bool proximityGridCheck(NetworkingPlayer sender, NetworkingPlayer player, float proximityDistance,
            float proximityModeUpdateFrequency) {
        ...
        private bool updateCountCheck(NetworkingPlayer sender, NetworkingPlayer player,
            float proximityModeUpdateFrequency) {
        ...
        public void Disconnect(NetworkingPlayer player, bool forced,
            ...
            if (player.IsDisconnecting || DisconnectingPlayers.Contains(player) ||
