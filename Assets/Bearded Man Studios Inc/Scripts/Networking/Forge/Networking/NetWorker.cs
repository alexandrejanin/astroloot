...
namespace BeardedManStudios.Forge.Networking {
    public abstract class NetWorker {
        ...
        internal bool ObjectCreatedRegistered {
            get { return _objectCreated != null; }
        }
        ...
        public Dictionary<uint, List<Action<NetworkObject>>> missingObjectBuffer =
            new Dictionary<uint, List<Action<NetworkObject>>>();
                ...
                if (forceId != 0) {
                    ...
                    if (!networkObject.RegisterOnce(id))
                        throw new BaseNetworkException(
                            "The supplied network object has already been assigned to a networker and has an id");
        ...
        internal void OnObjectCreateRequested(int identity, uint id, FrameStream frame,
        ...
        protected void OnMessageReceived(NetworkingPlayer player, FrameStream frame) {
            ...
            if (frame is Binary) {
                ...
                if (routerId == RouterIds.RPC_ROUTER_ID || routerId == RouterIds.BINARY_DATA_ROUTER_ID ||
                    routerId == RouterIds.CREATED_OBJECT_ROUTER_ID) {
        ...
        private void ExecuteRouterAction(byte routerId, NetworkObject networkObject, Binary frame,
        ...
        public static void EndSession() {
            ...
            Task.Queue(() => { EndingSession = false; }, 1000);
        }
        ...
        public Ping GeneratePing() {
            ...
            return new Ping(Time.Timestep, this is TCPClient, payload, Receivers.Server, MessageGroupIds.PING,
                this is BaseTCP);
        }
        ...
        protected Pong GeneratePong(DateTime time) {
            ...
            payload.BlockCopy<long>(ticks, sizeof(long));
            return new Pong(Time.Timestep, this is TCPClient, payload, Receivers.Target, MessageGroupIds.PONG,
                this is BaseTCP);
        }
        ...
        public static void RefreshLocalUdpListings(ushort portNumber = DEFAULT_PORT, int responseBuffer = 1000) {
            ...
            foreach (IPAddress ipAddress in GetLocalIPs()) {
                ...
                Task.Queue(() => {
                    ...
                    localListingsClient.Send(
                        new byte[] {
                            BROADCAST_LISTING_REQUEST_1, BROADCAST_LISTING_REQUEST_2, BROADCAST_LISTING_REQUEST_3
                        }, 3,
