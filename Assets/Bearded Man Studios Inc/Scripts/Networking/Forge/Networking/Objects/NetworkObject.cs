...
namespace BeardedManStudios.Forge.Networking {
    ...
    public abstract class NetworkObject {
        ...
        public delegate void CreateRequestEvent(NetWorker networker, int identity, uint id, FrameStream frame,
        ...
        public uint MyPlayerId {
            get { return Networker.Me.NetworkId; }
        }
        ...
        public static List<NetworkObject> NetworkObjects {
            get { return networkObjects; }
        }
        ...
        public bool IsServer {
            get { return Networker is IServer; }
        }
        ...
        private struct PendingLocalRPC {
            ...
            public override string ToString() {
                return string.Format("P [{0}], M [{1}], R [{2}], A [{3}]", TargetPlayer, MethodId, Receivers,
                    Args.Length);
            }
        }
        ...
        public NetworkObject(NetWorker networker, INetworkBehavior networkBehavior = null, int createCode = 0,
            ...
            if (networker is IServer)
                ...
                Binary createRequest = new Binary(CreateTimestep, useMask, data, Receivers.Server,
                    MessageGroupIds.CREATE_NETWORK_OBJECT_REQUEST, networker is BaseTCP,
                    RouterIds.NETWORK_OBJECT_ROUTER_ID);
        ...
        public NetworkObject(NetWorker networker, uint serverId, FrameStream frame) {
            ...
            if (networker is IServer) {
                ...
                Binary createdFrame = new Binary(Networker.Time.Timestep, Networker is TCPClient, createdByteData,
                    Receivers.Server, MessageGroupIds.GetId("NO_CREATED_" + NetworkId), Networker is BaseTCP,
                    RouterIds.CREATED_OBJECT_ROUTER_ID);
        ...
        private Binary CreateObjectOnServer(NetworkingPlayer skipPlayer, int targetHash = 0) {
            ...
            Binary createObject = new Binary(CreateTimestep, false, data, Receivers.All,
                MessageGroupIds.CREATE_NETWORK_OBJECT_REQUEST, Networker is BaseTCP,
                RouterIds.NETWORK_OBJECT_ROUTER_ID);
            ...
            Task.Queue(() => {
                lock (player) {
                    ...
                    foreach (NetworkObject obj in networkObjects) {
                        ...
                        ObjectMapper.Instance.MapBytes(targetData, obj.UniqueIdentity, 0, obj.NetworkId,
                            obj.CreateCode);
                    ...
                    if (targetData.Size > 0 && networker != null) {
                        Binary targetCreateObject = new Binary(timestep, false, targetData, Receivers.Target,
                            MessageGroupIds.CREATE_NETWORK_OBJECT_REQUEST, networker is BaseTCP,
                            RouterIds.ACCEPT_MULTI_ROUTER_ID);
        ...
        private void Initialize(uint id = 0, ulong timestep = 0) {
            ...
            if (Networker.PendCreates) {
                lock (PendingCreatesLock) {
                    if (Networker.PendCreates
                    ) // Check a second time in case Networker.PendCreates was changed while waiting for the lock
                    {
        ...
        public static void Flush(NetWorker target, List<int> remainingScenesToLoad = null,
            NetworkObjectEvent objectCreatedHandler = null) {
            lock (PendingCreatesLock) {
        ...
        public void RegisterRpc(string methodName, Action<RpcArgs> callback, params Type[] argumentTypes) {
            ...
            if (Rpcs.Count >= byte.MaxValue)
                throw new BaseNetworkException("You are only allowed to register " + byte.MaxValue +
                                               " Rpc methods per network object");
        ...
        private void SendBuffer(NetworkingPlayer player) {
            ...
            lock (rpcBuffer) {
                for (int i = 0; i < count; i++)
                    FinalizeSendRpc(rpcBuffer[i].data, rpcBuffer[i].receivers, rpcBuffer[i].methodId,
                        rpcBuffer[i].timestep, true, player);
            }
        }
        ...
        public void SendRpc(string methodName, Receivers receivers, params object[] args) {
            ...
            if (!rpcLookup.TryGetValue(methodName, out methodId))
                throw new Exception(
                    "Invalid method name supplied, this method is also obsolete so maybe this is a good time to update to the new non-string based methods");
        ...
        public void SendRpc(NetworkingPlayer targetPlayer, string methodName, Receivers receivers,
            params object[] args) {
            byte methodId;
        ...
        /// <param name="args">The input arguments for the method call</param>
        [Obsolete("Please use the SendRpc that takes the byte id argument instead for better performance")]
        public void SendRpc(NetworkingPlayer targetPlayer, string methodName, bool replacePrevious, Receivers receivers,
            params object[] args) {
            byte methodId;
        ...
        /// <param name="args">The input arguments for the method call</param>
        /// <returns></returns>
        public void SendRpc(NetworkingPlayer targetPlayer, byte methodId, bool replacePrevious, bool reliable,
            Receivers receivers, NetworkingPlayer sender, object[] args) {
            ...
            if (Networker is IServer) {
                ...
                if (targetPlayer == Networker.Me || receivers == Receivers.Server ||
                    receivers == Receivers.ServerAndOwner)
                    InvokeRpcOnSelfServer(methodId, sender, timestep, args);
                ...
                // that don't include itself
                                                          ...
                                                          (receivers != Receivers.Others &&
                                                           ...
                                                           receivers != Receivers.OthersProximity &&
                                                           receivers != Receivers.Target &&
                                                           receivers != Receivers.OthersProximityGrid))) {
                    InvokeRpcOnSelfServer(methodId, sender, timestep, args);
                }
            }
        }
        ...
        private void InvokeRpcOnSelfServer(byte methodId, NetworkingPlayer sender, ulong timestep, object[] args) {
            Rpcs[methodId].Invoke(new RpcArgs(args, new RPCInfo {SendingPlayer = sender, TimeStep = timestep}),
                sender == Networker.Me);
        }

        private void FinalizeSendRpc(BMSByte data, Receivers receivers, byte methodId, ulong timestep, bool reliable,
            NetworkingPlayer targetPlayer = null, NetworkingPlayer sender = null) {
            // Generate a binary frame with a router
            Binary rpcFrame = new Binary(timestep, Networker is TCPClient, data, receivers,
                MessageGroupIds.GetId("NO_RPC_" + NetworkId + "_" + methodId), Networker is BaseTCP,
                RouterIds.RPC_ROUTER_ID);
        ...
        public void SendBinaryData(BMSByte data, Receivers receivers, byte subRouter = 0, bool reliable = true,
            ...
            lock (sendBinaryData) {
                ...
                // Generate a binary frame with a router
                Binary frame = new Binary(Networker.Time.Timestep, Networker is TCPClient, sendBinaryData, receivers,
                    MessageGroupIds.GetId("NO_BIN_DATA_" + NetworkId), Networker is BaseTCP,
                    RouterIds.BINARY_DATA_ROUTER_ID);
                ...
                case DIRTY_FIELD_SUB_ROUTER_ID:
                    ...
                    if (Networker is IClient || frame.Sender == Owner) {
                        ...
                        if (Networker is IServer) {
                            ...
                            if (data != null)
                                SendBinaryData(data, ProximityBasedFields ? ProximityBasedFieldsMode : Receivers.All,
        ...
        public static void CreateNetworkObject(NetWorker networker, NetworkingPlayer player, Binary frame) {
            ...
            if (networker is IServer) {
                ...
                if (Factory != null) {
                    Factory.NetworkCreateObject(networker, identity, 0, frame, (obj) => { networkObjects.Add(obj); });
                }
        ...
        public void Destroy(int timeInMilliseconds = 0) {
            if (timeInMilliseconds > 0) {
                Task.Queue(() => { Destroy(false); }, timeInMilliseconds);
            } else
                Destroy(false);
        }
        ...
        private void Destroy(bool remoteCall) {
            if ((IsOwner && !remoteCall) || Networker is IServer)
                SendBinaryData(null, Receivers.Others, DESTROY_SUB_ROUTER_ID,
                    skipOwner: Networker is IServer && remoteCall);
