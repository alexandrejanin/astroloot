...
namespace BeardedManStudios.Forge.Networking {
    public class UDPClient : BaseUDP, IClient {
        ...
        public NetworkingPlayer Server {
            get { return server; }
        }
        ...
        public override void Disconnect(bool forced) {
            ...
            lock (Client) {
                if (forced)
                    ...
                    var frame = new ConnectionClose(Time.Timestep, false, Receivers.Server, MessageGroupIds.DISCONNECT,
                        false);
                    Send(frame, true);
            ...
            try {
                ...
                while (IsBound) {
                    ...
                    if (groupEP.Address != Server.IPEndPointHandle.Address &&
                        ...
                        if (packet.Size == 1 &&
                            (packet[0] == SERVER_BROADCAST_CODE || packet[1] == CLIENT_BROADCAST_CODE)) { } else if (
                            packet.Size.Between(2, 4) && packet[0] == BROADCAST_LISTING_REQUEST_1 &&
                    ...
                    if (!headerExchanged) {
                        if (Websockets.ValidateResponseHeader(headerHash, packet.CompressBytes())) {
                            ...
                            Send(
                                Text.CreateFromString(Time.Timestep, InstanceGuid.ToString(), false, Receivers.Server,
