    ...
    public class ObjectMapper {
        ...
        public object MapBasicType(Type type, BMSByte stream) {
            if (type == typeof(sbyte))
                ...
                throw new BaseNetworkException("The type " + type.ToString() +
                                               " is not allowed to be sent over the Network (yet)");
        }
        ...
        protected virtual byte[] GetBytesArray(object o, Type type) {
            if (type == typeof(string)) {
                ...
                Buffer.BlockCopy(((BMSByte) o).byteArr, ((BMSByte) o).StartIndex(), bytes, bytesSize.Length,
                    ((BMSByte) o).Size);
                return bytes;
            } else if (type.IsEnum)
