...
namespace BeardedManStudios.Forge.Networking.Frame {
    public abstract class FrameStream : ICloneable {
        ...
        /// <param name="routerId">A byte that can be used to route data within a user program</param>
        public FrameStream(ulong timestep, bool useMask, byte[] payload, Receivers receivers, int groupId,
            bool isStream, byte routerId = 0) {
            if (payload == null)
                // TODO:  Throw frame exception
                throw new BaseNetworkException(
                    "The payload for the frame is not allowed to be null, otherwise use other constructor");
        ...
        /// <param name="payload">The new frame initial data data</param>
        /// <param name="routerId">A byte that can be used to route data within a user program</param>
        public FrameStream(ulong timestep, bool useMask, BMSByte payload, Receivers receivers, int groupId,
            bool isStream, byte routerId = 0) {
            if (payload == null)
                // TODO:  Throw frame exception
                throw new BaseNetworkException(
        ...
        /// </summary>
        private void CreateFrame(bool useMask, ulong timestep, byte[] payload, Receivers receivers, int groupId,
            byte routerId, bool isStream) {
        ...
        public byte[] GetData(bool makeReliable = false, NetworkingPlayer player = null) {
            if (makeReliable) {
                ...
                reliableCloneData.InsertRange(StreamData.Size - (sizeof(ulong) * 2),
                    BitConverter.GetBytes(UniqueReliableId));
