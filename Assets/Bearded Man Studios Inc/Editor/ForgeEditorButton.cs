...
namespace BeardedManStudios.Forge.Networking.UnityEditor {
    ...
    public class ForgeEditorButton {
        ...
        public bool IsNetworkBehavior {
            get { return ButtonName.EndsWith("Behavior"); }
        }

        public bool IsNetworkObject {
            get { return ButtonName.EndsWith("NetworkObject"); }
        }
        ...
        public bool RenderExposed(Action callback = null, bool ignoreButton = false) {
            ...
            if (TiedObject == null) {
                ...
                ButtonName = GUILayout.TextField(ButtonName,
                    GUILayout.Width(ForgeNetworkingEditor.Instance.position.width - 50));
            ...
            if (CanRenderRPCS) {
                ...
                if (TiedObject != null)
                    EditorGUILayout.HelpBox(
                        "Re-arranging the RPC Arguments will require you to manually update your deriving code to use the new logic, please be aware of this.",
                        MessageType.Warning);
        ...
        public ValidationResult ValidateSetup() {
            ...
            for (int i = 0; i < ClassVariables.Count; ++i) {
                ...
                if (variableNames.Contains(checkedName)) {
                    result.ReportValidationError(String.Format("Duplicate element \"{0}\" found in \"{1}\"",
                        checkedName, ButtonName));
                }
                ...
                switch (ClassVariables[i].FieldName) {
                    ...
                    case "_dirtyFields":
                    case "dirtyFields":
                        result.ReportValidationError(String.Format(
                            "Field \"{0}\" conflicts with a field name reserved for Forge Networking Remastered",
                            checkedName));
                        break;
                }
                ...
                if (checkedName.EndsWith("Changed")) {
                    ...
                    if (variableNames.Contains(duplicate)) {
                        result.ReportValidationError(String.Format("Field \"{0}\" conflicts with Changed event of {1}",
                ...
                if (checkedName.EndsWith("Interpolation")) {
                    ...
                    if (variableNames.Contains(duplicate)) {
                        result.ReportValidationError(String.Format(
                            "Field \"{0}\" conflicts with Interpolation field of {1}", checkedName, duplicate));
                    }
                }
                ...
                if (ForgeNetworkingEditor.IsValidName(checkedName))
                    ...
                    result.ReportValidationError(String.Format("Invalid identifier for Field \"{0}\" in \"{1}\"",
                        checkedName, ButtonName));
                }
            }
            ...
            for (int i = 0; i < RPCVariables.Count; ++i) {
                ...
                if (ForgeNetworkingEditor.IsValidName(checkedName))
                    ...
                    result.ReportValidationError(String.Format("Invalid identifier for RPC \"{0}\" in \"{1}\"",
                        checkedName, ButtonName));
                }
                ...
                switch (RPCVariables[i].FieldName.ToLower()) {
                    ...
                    case "networkcreateobject":
                        result.ReportValidationError(String.Format(
                            "RPC \"{0}\" conflicts with a method name reserved for Forge Networking Remastered",
                            checkedName));
                        break;
                }
                ...
                if (RPCVariables[i].FieldName.StartsWith("get_") ||
                    ...
                    result.ReportValidationError(String.Format("RPC \"{0}\" cannot start with \"get_\" or \"set_\"",
                        checkedName));
                    break;
                }
            }
            ...
            for (int i = 0; i < RewindVariables.Count; ++i) {
                ...
                if (ForgeNetworkingEditor.IsValidName(checkedName))
                    ...
                    result.ReportValidationError(String.Format("Invalid identifier for Rewind \"{0}\" in \"{1}\"",
                        checkedName, ButtonName));
                }
            }
