...
#pragma warning restore 0067
        [ForgeGeneratedField] private bool _alive;
        ...
        [ForgeGeneratedField] private Vector2 _position;
        ...
        [ForgeGeneratedField] private Vector2 _aimingPosition;
        public event FieldEvent<Vector2> aimingPositionChanged;

        public InterpolateVector2 aimingPositionInterpolation =
            new InterpolateVector2() {LerpT = 0.25f, Enabled = true};
        ...
        [ForgeGeneratedField] private int _health;
        ...
        public override void InterpolateUpdate() {
            ...
            if (aliveInterpolation.Enabled &&
                !aliveInterpolation.current.UnityNear(aliveInterpolation.target, 0.0015f)) {
                _alive = (bool) aliveInterpolation.Interpolate();
                //RunChange_alive(aliveInterpolation.Timestep);
            }

            if (positionInterpolation.Enabled &&
                !positionInterpolation.current.UnityNear(positionInterpolation.target, 0.0015f)) {
                _position = (Vector2) positionInterpolation.Interpolate();
                //RunChange_position(positionInterpolation.Timestep);
            }

            if (aimingPositionInterpolation.Enabled &&
                !aimingPositionInterpolation.current.UnityNear(aimingPositionInterpolation.target, 0.0015f)) {
            ...
            if (healthInterpolation.Enabled &&
                !healthInterpolation.current.UnityNear(healthInterpolation.target, 0.0015f)) {
        ...
        public PlayerNetworkObject() : base() {
            Initialize();
        }

        public PlayerNetworkObject(NetWorker networker, INetworkBehavior networkBehavior = null, int createCode = 0,
            byte[] metadata = null) : base(networker, networkBehavior, createCode, metadata) {
            Initialize();
        }

        public PlayerNetworkObject(NetWorker networker, uint serverId, FrameStream frame) : base(networker, serverId,
            frame) {
            Initialize();
        }
